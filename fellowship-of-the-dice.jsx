...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await window.storage.get(STORAGE_KEY); if (r) setAvail(JSON.parse(r.value)); } catch {}
    })();
  }, []);

  const persist = async (a) => {
    setAvail(a);
    try { await window.storage.set(STORAGE_KEY, JSON.stringify(a)); } catch {}
  };
